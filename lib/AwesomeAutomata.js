...
AwesomeAutomata.prototype.next = function next(input, callback) {
  ...
  if (typeof nextState.accept === 'function') {
    this._accept(nextState.accept(this._currentState, this._previouslyVisitedStates));
  }

  this._transition(input, nextState, transition); // Visit the next state
...
AwesomeAutomata.prototype._transition = function transition(input, nextState, transition) {
  ...
  if (typeof transition.accept === 'function') {
    this._accept(transition.accept(this._currentState, this._previouslyVisitedStates))
  }
