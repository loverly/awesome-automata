 ...
 */
AwesomeAutomata.prototype.throwFatalError = function throwFatalError(msg) {
 ...
 */
AwesomeAutomata.prototype.emitError = function emitError(msg) {
...
AwesomeAutomata.prototype.addStates = function addStates(states) {
  if (!(states instanceof Array)) {
    this.throwFatalError('addStates() must be called with an array of states');
  }
...
AwesomeAutomata.prototype.addState = function addState(stateConfig) {
  // Add an onAccept() method so the state can determine how to trigger return
  // events
  stateConfig.onAccept = this._accept.bind(this);

  var state = new State(stateConfig);

  if (this._states[state.getName()]) {
    this.throwFatalError('The state "' + state.name + '" has already been defined.');
  }
  ...
  if (state.isInitial()) {
    if (this._rootNode) {
      this.throwFatalError('Cannot redefine root node with state: ' + state.getName());
    }
...
AwesomeAutomata.prototype.next = function next(input, callback) {
  if (!this._currentState) {
    this.throwFatalError('Cannot start processing data without a starting state.');
  }

  var currentState = this._currentState;
  var nextState = this._findNextState(input, currentState, this._previousState);
  ...
  // Make successive calls to next asynchronous - doesn't really do anything
 ...
 * Update
 */
AwesomeAutomata.prototype._transition = function transition(input, nextState) {
...
/**
 * Emit a returned object from the machine from an accepting state
 */
AwesomeAutomata.prototype._accept = function accept(state) {
  this.emit(
    'return',
    state.accept(this._currentState, this._previouslyVisitedStates)
  );
};
...
AwesomeAutomata.prototype.reset = function reset() {
  ...
  var path = this._previouslyVisitedStates;

  this._currentState = this._rootNode;
  this._previouslyVisitedStates = [];

  this.emit('reset', {finalState: finalState.getName(), path: path});
  return this;
};
